...
FrameMetadata = calllib('libND2ReadSDk', 'Lim_FileGetFrameMetadata', FilePointer,0);
TestLength=3000;
...
calllib('libND2ReadSDk', 'Lim_FileFreeString', FrameMetadata);
...
for i=1:size(Num,2)
    ...
    FrameMetadata = calllib('libND2ReadSDk', 'Lim_FileGetFrameMetadata', FilePointer,Num(i)-1);
    setdatatype(FrameMetadata, 'uint8Ptr', TestLength)
    FrameMetadataValue = FrameMetadata.Value';
    FrameMetadataLength = find(FrameMetadataValue == 0, 1);
    FrameMetadataJson = char(FrameMetadataValue(1:FrameMetadataLength - 1));
